...
/**
 * Test class for {@link XyChain}.
 */
class XyChainTest {
	
	/**
	 * A generated NINE CLASSIC puzzle (band TWELVE, seed 0) whose solve applies the technique 156 time(s).
	 */
	...
	void technique_returnsTheMatchingConstant() {
		assertEquals(Technique.XY_CHAIN, new XyChain().technique());
	}
	...
	void find_puzzleThatNeedsIt_appliesItDuringTheSolve() {
		...
		assertAll(
			...
			() -> assertTrue(report.count(Technique.XY_CHAIN) >= 1,
				"The fixture must still need XY_CHAIN, but the solve used " + report.usage())
		);
	}
	...
	void find_firstStateItApplies_agreesWithTheTrueSolution() {
		...
		Optional<Deduction> deduction = StrategyFixtures.firstFiring(new XyChain(), puzzle);
		
		assertTrue(deduction.isPresent(), "XY_CHAIN never applied anywhere in the fixture's solve");
		assertEquals(Technique.XY_CHAIN, deduction.orElseThrow().technique());
	...
	void find_emptyGrid_returnsEmpty() {
		...
		assertTrue(new XyChain().find(grid).isEmpty());
	}
	...
	void find_nullGrid_throwsNullPointerException() {
		assertThrows(NullPointerException.class, () -> new XyChain().find(null));
	}
}
