...
/**
 * Test class for {@link AlsChain}.
 */
class AlsChainTest {
	
	/**
	 * A generated NINE CLASSIC puzzle (band FOURTEEN, seed 7) whose solve applies the technique 32 time(s).
	 */
	...
	void technique_returnsTheMatchingConstant() {
		assertEquals(Technique.ALS_CHAIN, new AlsChain().technique());
	}
	...
	void find_puzzleThatNeedsIt_appliesItDuringTheSolve() {
		...
		assertAll(
			...
			() -> assertTrue(report.count(Technique.ALS_CHAIN) >= 1,
				"The fixture must still need ALS_CHAIN, but the solve used " + report.usage())
		);
	}
	...
	void find_firstStateItApplies_agreesWithTheTrueSolution() {
		...
		Optional<Deduction> deduction = StrategyFixtures.firstFiring(new AlsChain(), puzzle);
		
		assertTrue(deduction.isPresent(), "ALS_CHAIN never applied anywhere in the fixture's solve");
		assertEquals(Technique.ALS_CHAIN, deduction.orElseThrow().technique());
	...
	void find_emptyGrid_returnsEmpty() {
		...
		assertTrue(new AlsChain().find(grid).isEmpty());
	}
	...
	void find_nullGrid_throwsNullPointerException() {
		assertThrows(NullPointerException.class, () -> new AlsChain().find(null));
	}
}
