...
/**
 * Test class for {@link NakedPair}.
 */
class NakedPairTest {
	
	/**
	 * A generated, uniquely solvable 9x9 puzzle in whose fresh candidate grid a naked pair removes the digit 9
	 * from cells 18 and 21.
	 */
	...
	private static final int[] EXPECTED_CELLS = { 18, 21 };
	private static final int[] EXPECTED_DIGITS = { 9, 9 };
	...
	@Test
	void find_unitWithANakedPair_returnsTheExpectedEliminations() {
		CandidateGrid grid = new CandidateGrid(puzzle(PAIR_PRESENT));
		
		Optional<Deduction> deduction = new NakedPair().find(grid);
		
		assertEquals(Optional.of(new Deduction.Eliminations(Technique.NAKED_PAIR, EXPECTED_CELLS, EXPECTED_DIGITS)), deduction);
	}
	...
	void find_eliminations_removeNoDigitOfTheTrueSolution() {
		...
		Deduction.Eliminations eliminations = (Deduction.Eliminations) new NakedPair().find(new CandidateGrid(puzzle)).orElseThrow();
	...
	void find_eliminationsApplied_removeTheCandidates() {
		CandidateGrid grid = new CandidateGrid(puzzle(PAIR_PRESENT));
		Deduction deduction = new NakedPair().find(grid).orElseThrow();
		
		boolean changed = deduction.applyTo(grid);
		
		assertAll(
			() -> assertTrue(changed),
			() -> assertFalse(grid.hasCandidate(18, 9)),
			() -> assertFalse(grid.hasCandidate(21, 9))
		);
	}
	
	@Test
	void find_emptyGridWithoutAnyPair_returnsEmpty() {
		CandidateGrid grid = new CandidateGrid(emptyNine());
		
		assertTrue(new NakedPair().find(grid).isEmpty());
	}
	
	@Test
	void technique_always_isNakedPair() {
		assertEquals(Technique.NAKED_PAIR, new NakedPair().technique());
	}
}
