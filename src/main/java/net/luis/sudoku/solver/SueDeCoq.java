...
public final class SueDeCoq implements TechniqueStrategy {
	...
	public Optional<Deduction> find(CandidateGrid grid) {
		...
		for (int region = 0; region < grid.partition().regionCount(); region++) {
			for (int line = 0; line < n; line++) {
				Optional<Deduction> byRow = this.scan(grid, region, grid.rowCells(line));
				if (byRow.isPresent()) {
					return byRow;
				}
				
				Optional<Deduction> byColumn = this.scan(grid, region, grid.columnCells(line));
	...
	private Optional<Deduction> scan(CandidateGrid grid, int region, int[] lineCells) {
		...
		for (int lineCell : lineCells) {
			...
			for (int regionCell : regionCells) {
				...
				Optional<Deduction> found = this.eliminate(grid, intersection, lineCells, regionCells, lineCell, regionCell, digits);
				if (found.isPresent()) {
					return found;
				}
			}
	...
	private Optional<Deduction> eliminate(CandidateGrid grid, int[] intersection, int[] lineCells, int[] regionCells, int lineCell, int regionCell, int digits) {
		...
		builder.sortByCell();
		return builder.build(Technique.SUE_DE_COQ);
	}
